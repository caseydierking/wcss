...
var myGamePiece;
var appleArr = [];
var cycleThroughPhotos = 0; //To add components to the canvas, follow the steps below:
...
function startGame() {
  ...
  myGameArea.start();
  myGamePiece = new component(130, 100, "images/horse-movements/horse-all.png", 10, 120, "image", 0, 0, 250, 188); // init apples
...
var myGameArea = {
  ...
  start: function start() {
    ...
    document.getElementById("game-div").appendChild(this.canvas); //document.body.insertBefore(this.canvas, document.body.childNodes[0]);
    ...
    window.addEventListener('keyup', function (e) {
      if (myGameArea.keyArr.includes(e.keyCode)) {
        myGameArea.keyArr.splice(myGameArea.keyArr.indexOf(e.keyCode), 1); // if a key is released but others are still held, make the active key the most recent held one
...
function component(width, height, color, x, y, type, cropx, cropy, cropwidth, cropheight) {
  ...
  this.newPos = function () {
    ...
    this.x += this.speedX;
    this.y += this.speedY; // if horse close to the apple, remove apple from appleArr

    appleArr.forEach(function (apple, index, object) {
      if (Math.abs(_this.x - apple.x) < 100 && Math.abs(_this.y - apple.y) < 100) {
...
function moveleft() {
  ...
  if (cycleThroughPhotos <= 5) {
    myGamePiece.cropx = 250; // console.log(myGamePiece);
  } else if (cycleThroughPhotos < 10) {
    myGamePiece.cropx = 500;
  } else if (cycleThroughPhotos < 15) {
    myGamePiece.cropx = 750;
